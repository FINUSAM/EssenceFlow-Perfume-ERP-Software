...
const Purchasing: React.FC = () => {
  ...
  const updateItemRow = (index: number, updates: Partial<PurchaseItem>) => {
    ...
    if (updates.inventoryItemId) {
      const invItem = inventory.find(i => i.id === updates.inventoryItemId);
  ...
  return (
    <div className="space-y-6 md:space-y-8 animate-fadeIn">
      ...
      <div className="hidden lg:block bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-[2.5rem] shadow-xl overflow-hidden border border-white dark:border-slate-800">
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-slate-100/50 dark:divide-slate-800/50">
            {purchases.map(order => (
              <tr key={order.id} className="hover:bg-gold-50/20 dark:hover:bg-slate-800/30 transition-all">
                ...
                <td className="p-6 text-sm font-medium text-slate-700 dark:text-slate-200">{vendors.find(v => v.id === order.vendorId)?.name || 'Unknown Entity'}</td>
                ...
                <td className="p-6 text-right pr-10">
                  <button onClick={() => { setConfirmDeleteId(order.id); setDeleteError(null); }} className="p-3 text-rose-400 hover:text-rose-600 transition-colors">
      ...
      <div className="lg:hidden grid grid-cols-1 sm:grid-cols-2 gap-4 pb-20">
        {purchases.map(order => (
          <div key={order.id} className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl p-6 rounded-[2.5rem] shadow-sm border border-white dark:border-slate-800 transition-all">
            ...
            <p className="text-[10px] font-bold text-gold-600 truncate">{vendors.find(v => v.id === order.vendorId)?.name || 'Unknown Entity'}</p>
        ...
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-xl flex items-end sm:items-center justify-center p-0 sm:p-4 z-[100] overflow-y-auto">
          <div className="bg-white dark:bg-slate-900 rounded-t-[3rem] sm:rounded-[3rem] p-8 md:p-14 max-w-4xl w-full shadow-2xl animate-scaleUp text-slate-900 dark:text-white pb-12 overflow-y-auto max-h-[95vh] sm:max-h-[90vh] no-scrollbar">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
              <div>
                <h2 className="text-3xl md:text-4xl font-serif font-bold">Restock Order</h2>
            ...
            <div className="space-y-8">
              ...
              <div>
                ...
                <div className="space-y-4">
                  {orderItems.map((item, idx) => {
                    const selectedInvItem = inventory.find(i => i.id === item.inventoryItemId);
                    return (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                          <div className="md:col-span-6">
                            ...
                            <select className="w-full p-3 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 text-xs font-bold" value={item.inventoryItemId} onChange={e => updateItemRow(idx, { inventoryItemId: e.target.value })}>
                              ...
                              {inventory.filter(i => !selectedVendorId || i.vendorId === selectedVendorId).map(i => <option key={i.id} value={i.id}>{i.name} ({i.category})</option>)}
                            </select>
                          </div>
              ...
              <div className="flex flex-col sm:flex-row justify-end gap-4 pt-10 border-t border-slate-50 dark:border-slate-800">
                <button onClick={() => { setShowNewOrder(false); setErrorMessage(''); setOrderItems([]); }} className="px-10 py-4 text-slate-400 font-black uppercase tracking-widest text-[10px]">Discard Order</button>
                <button onClick={handleCreatePurchase} className="w-full sm:w-auto px-12 py-5 bg-gold-600 text-white rounded-[1.5rem] font-black uppercase tracking-widest text-[10px] shadow-2xl active:scale-95 transition-all">Verify & Import Inventory</button>
              </div>
            </div>
          </div>
        </div>
