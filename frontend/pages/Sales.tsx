    ...
    try {
      ...
      const finalSale = {
        ...
        items: saleItems.map(si => ({
          ...
          unitPrice: products.find(p => p._id === si.productId)?.sellingPrice || 0,
          total: (products.find(p => p._id === si.productId)?.sellingPrice || 0) * si.quantity
        })),
        subtotal,
        discount,
  ...
  return (
    <div className="space-y-6 md:space-y-8 animate-fadeIn">
      ...
      <div className="hidden lg:block bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-[2.5rem] shadow-xl overflow-hidden border border-white dark:border-slate-800">
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-slate-100/50 dark:divide-slate-800/50">
            {sales.map(sale => (
              <tr key={sale._id} className={`${sale.status === 'VOIDED' ? 'opacity-40' : 'hover:bg-gold-50/30 dark:hover:bg-slate-800/30'} transition-all`}>
      ...
      <div className="lg:hidden grid grid-cols-1 sm:grid-cols-2 gap-4 pb-20">
        {sales.map(sale => (
          <div key={sale._id} className={`bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl p-6 rounded-[2.5rem] shadow-sm border border-white dark:border-slate-800 transition-all ${sale.status === 'VOIDED' ? 'opacity-50' : ''}`}>
            ...
            {sale.status === 'COMPLETED' && (
              <button onClick={() => setConfirmVoidId(sale._id)} className="w-full py-3 border-2 border-dashed border-rose-200 dark:border-rose-900/30 rounded-2xl flex items-center justify-center gap-2 group transition-all active:bg-rose-50 dark:active:bg-rose-950/20">
                    ...
                    {saleItems.map((si, idx) => (
                      <div key={idx} className="flex gap-4 items-center bg-slate-50 dark:bg-slate-900/50 p-5 rounded-2xl border border-slate-100 dark:border-slate-800/50 shadow-sm relative">
                        <select className="flex-1 p-2 bg-transparent text-slate-900 dark:text-white text-sm font-bold outline-none" value={si.productId} onChange={e => { const items = [...saleItems]; items[idx].productId = e.target.value; setSaleItems(items); }}>
                          ...
                          {products.map(p => <option key={p._id} value={p._id} disabled={p.currentStock <= 0}>{p.name} (Stock: {p.currentStock})</option>)}
                        </select>
