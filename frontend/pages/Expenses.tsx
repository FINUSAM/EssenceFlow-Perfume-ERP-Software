...
const Expenses: React.FC = () => {
  ...
  const handleCreateOrUpdate = async (e: React.FormEvent) => {
    e.preventDefault();
    if (editingExpense) {
      await api.updateExpense(editingExpense._id, newExp);
    } else {
  ...
  return (
    <div className="space-y-6 md:space-y-8 animate-fadeIn">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 pb-24 md:pb-0">
        ...
        {expenses.map(exp => (
          <div key={exp._id} className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl p-7 rounded-[2.5rem] shadow-sm border border-white dark:border-slate-800 relative transition-all group">
            <div className="flex justify-between items-start mb-6">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="p-2.5 bg-slate-50 dark:bg-slate-800 rounded-xl text-slate-300 hover:text-gold-500 transition-all border border-slate-100 dark:border-slate-700"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                </button>
                <button
                  onClick={() => { setConfirmDeleteId(exp._id); setDeleteError(null); }}
      ...
      {showAdd && (
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-xl flex items-end sm:items-center justify-center p-0 sm:p-4 z-[100]">
          <div className="bg-white dark:bg-slate-900 rounded-t-[3rem] sm:rounded-[3rem] p-8 md:p-14 max-w-md w-full shadow-2xl animate-scaleUp text-slate-900 dark:text-white pb-12 sm:pb-14">
            <h2 className="text-3xl font-serif font-bold mb-1">{editingExpense ? 'Modify Log' : 'Overhead Log'}</h2>
            <p className="text-slate-400 text-[10px] mb-10 tracking-widest uppercase font-black">{editingExpense ? 'Revise Expenditure Record' : 'Operational Expenditure Record'}</p>
            <form onSubmit={handleCreateOrUpdate} className="space-y-6">
              ...
              <div className="flex flex-col sm:flex-row justify-end gap-4 pt-10 border-t border-slate-50 dark:border-slate-800">
                <button type="button" onClick={handleCloseModal} className="px-8 py-4 text-slate-400 font-black uppercase tracking-widest text-[10px]">Discard</button>
                <button type="submit" className="w-full sm:w-auto px-12 py-5 bg-gold-600 text-white rounded-[1.5rem] font-black uppercase tracking-widest text-[10px] shadow-2xl active:scale-95 transition-all">{editingExpense ? 'Propose Edit' : 'Log Expenditure'}</button>
              </div>
            </form>
          </div>
        </div>
