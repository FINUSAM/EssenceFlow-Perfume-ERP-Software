...
const Inventory: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    setInventory(inv);
    setVendors(vends);
    if (vends.length > 0 && !newItem.vendorId) setNewItem(prev => ({ ...prev, vendorId: vends[0].id }));
  };
  ...
  const handleAddOrEdit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (editingItem) {
      await api.updateInventory(editingItem.id, newItem);
    } else {
  ...
  return (
    <div className="space-y-6 md:space-y-8 animate-fadeIn">
      ...
      <div className="hidden lg:block bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-[2.5rem] shadow-xl overflow-hidden border border-white dark:border-slate-800">
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-slate-100/50 dark:divide-slate-800/50">
            {filtered.map(item => (
              <tr key={item.id} className="hover:bg-gold-50/30 dark:hover:bg-slate-800/30 transition-all">
                <td className="p-6">
                ...
                <td className="p-6 text-right pr-10">
                  <div className="flex gap-4 justify-end">
                    ...
                    <button 
                      onClick={() => { setConfirmDeleteId(item.id); setDeleteError(null); }} 
                      className={`p-3 rounded-xl transition-all ${confirmDeleteId === item.id ? 'bg-red-500 text-white' : 'bg-rose-50 dark:bg-rose-900/10 text-rose-400 hover:text-rose-600'}`}
                    >
      ...
      <div className="lg:hidden grid grid-cols-1 sm:grid-cols-2 gap-4 pb-20">
        {filtered.map(item => (
          <div key={item.id} className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl p-6 rounded-[2.5rem] shadow-sm border border-white dark:border-slate-800 relative transition-all active:scale-[0.98]">
